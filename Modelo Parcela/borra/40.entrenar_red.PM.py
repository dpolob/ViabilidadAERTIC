...
def main(dataset, horastrain, generalmodel, mmscaler):
    ...
    red_parcela = Red_Parcela((24 + 24 + 1), 24)
    ...
    train_dataloader = DataLoader(dataset=train_ds, shuffle=True, batch_size=1)

    # preentramiento
    writer = SummaryWriter('runs/pre_training/MP')
    ...
    train_dataloader = DataLoader(dataset=train_ds, shuffle=False, batch_size=50)

    # funcion_RMS = nn.MSELoss()
    temp_scaler = pickle.load(open(mmscaler, 'rb'))

    writer = SummaryWriter('runs/deployed/MP')
    batch = horastrain
    for x_train_parcela, y_train_parcela in train_dataloader:
        x_train_parcela = x_train_parcela.type(torch.float)
        y_train_parcela = y_train_parcela.type(torch.float)
        ...
        red_parcela.train()
        train_ds_recursivo = TemperaturaDataSet(train[0:batch, :], red_general) # el resto
        train_dataloader_recursivo = DataLoader(dataset=train_ds_recursivo, shuffle=False, batch_size=50)
