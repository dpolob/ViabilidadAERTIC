...
class TemperaturaDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return (np.concatenate((general.detach().numpy(), self.datos[index, 180:-24]), 0), self.datos[index, -24:])



@click.command()
@click.option('--dataset', prompt='Ruta del dataset a validar entrenar: ', default='.\\dataset.csv')
@click.option('--horastrain', prompt='Horas a preentrenar: ', default=2160)
...
def main(dataset, horastrain, generalmodel, mmscaler):
    ...
    train_dataloader = DataLoader(dataset=train_ds, shuffle=True, batch_size=100)
    ...
    for x_train_parcela, y_train_parcela in train_dataloader:
        ...
        red_parcela.eval()
        y_pred_parcela = red_parcela(x_train_parcela)
        RMS = funcion_RMS(y_pred_parcela, y_train_parcela)
